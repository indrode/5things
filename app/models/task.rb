...
class Task < ActiveRecord::Base
  ...
  def self.find_batch(id, from, to)
    find(:all, :order => 'duedate, ordinal ASC', :conditions => ["duedate IN (?) AND tasklist_id = ?", (from..to), id ])
  end
  ...
  def self.find_incomplete_batch(id, from, to)
    find(:all, :order => 'duedate, ordinal ASC', :conditions => ["duedate IN (?) AND tasklist_id = ? AND completed=0", (from..to), id ])
  end
  
  # no use for this yet
  def self.search(search)
    search_condition = "%" + search + "%"
    find(:all, :conditions => ['body LIKE ?', search_condition])
  end

  # finds all past tasks that are not completed
  def self.find_overdue(id)
    find(:all, :order => 'duedate, ordinal ASC', :conditions => ["duedate > (?) AND duedate < (?) AND completed = 0 AND tasklist_id = ?", NEVER, Time.zone.now.to_date, id])
  end

  # find all tasks that have not been assigned to a date
  def self.find_unassigned(id)
    find(:all, :order => 'duedate, ordinal ASC', :conditions => ["duedate IN (?) AND tasklist_id = ?", NEVER, id])
  end

  # find all tasks that are assigned to a date
  def self.find_assigned(id)
    find(:all, :order => 'duedate, ordinal ASC', :conditions => ["duedate > (?) AND tasklist_id = ?", NEVER, id])    
  end
  
  def self.find_upcoming(id)
    find(:all, :order => 'duedate, ordinal ASC', :conditions => ["duedate >= (?) AND tasklist_id = ?", Time.zone.now.to_date, id])    
  end
    
  # find all past dates  
  def self.find_past(id)
    find(:all, :order => 'duedate, ordinal ASC', :conditions => ["duedate > (?) AND duedate < (?) AND tasklist_id = ?", NEVER, Time.zone.now.to_date, id])
  end
